...
# здесь просто задаём цвета. Они не соответствуют своим названиям, но главное сгруппировано в одном месте
...
# поле игры - состоит из трёх частей: карта где расставлены корабли игрока,
...
class Field(object):
    ...
    # Рисуем поле. Здесь отрисовка делится на две части,
    #  т.к. отрисовка весов клеток идёт по другому
    ...
    # функция проверяет: помещается ли корабль на конкретную позицию
    #   конкретного поля
    # будем использовать при расстановке кораблей, 
    #   а также при вычислении веса клеток:
    ...
    # когда корабль уничтожен необходимо пометить все клетки 
    #   вокруг него сыгранными (Cell.miss_cell)
    # а все клетки корабля - уничтоженными (Cell.destroyed_ship)
    ...
    # добавление корабля: пробегаемся от позиции x y корабля по его высоте и
    #  ширине - помечаем на поле эти клетки
    # параметр element - сюда мы передаём к какой части поля мы обращаемся:
    #  основная, радар или вес
    def add_ship_to_field(self, ship, element):
        ...
        for p_x in range(x, x + height):
            for p_y in range(y, y + width):
                ...
                # таким образом, мы, обращаясь к клетке, всегда можем 
                # получить текущее HP корабля
                field[p_x][p_y] = ship
    
    # функция возвращает список координат с самым большим коэфициентом шанса попадения
    def get_max_weight_cells(self):
        weights = {}
        max_weight = 0
        # просто пробегаем по всем клеткам и заносим их в словарь
        #  с ключом, который является значением в клетке
        # заодно запоминаем максимальное значение, далее просто берём
        #  из словаря список координат с этим
...
class Game(object):
    ...
    def add_player(self, player):
        # при добавлении игрока создаём для нег поле
    ...
    def ships_setup(self, player):
        ...
        for ship_size in Game.ships_rules:
            ...
            while True:
                ...
                # пользователь ввёл какую-то ерунду - функция возвратит нули, значит, без вопросов делаем continue
                # фактически просто просим ещё раз ввести координаты
                if x + y + r == 0:
                    continue
